    ...
    <div class="scan-indicator-popup">
      ...
      <ul class="scan-indicator-list">
        <li v-for="job in activeJobs" :key="job.id" class="scan-indicator-job">
          <span class="scan-job-summary">
            <span class="scan-job-name" :title="scanJobName(job)">{{ scanJobName(job) }}</span>
...
function scanJobName(job: ScanJob) {
  return job.library_name || `Library ${job.library_id}`;
}
...
function scanCompactBreakdownLabel(job: ScanJob) {
  ...
  if (job.processed_images || job.total_estimated_images) {
    parts.push(t('scan.imagesCompact', { count: job.processed_images.toLocaleString() }));
  }
  const thumbnailImageTotal = Math.max(job.processed_images, job.thumbnail_ready_images);
  if (thumbnailImageTotal > 0) {
    parts.push(t('scan.thumbnailsCompact', { count: job.thumbnail_ready_images.toLocaleString(), total: thumbnailImageTotal.toLocaleString() }));
  }
  return parts.join(' · ');
}
</script>
